...
def _xml_paths(*candidates: str) -> list[str]:
...
class Phase1RetrievalTests(unittest.TestCase):
    def test_local_index_prefers_the_relevant_article(self) -> None:
        ...
        hits = index.search("renewable energy ev charging bangladesh", top_k=3)

        self.assertEqual(len(hits), 3)
        self.assertIn("sustainable transportation", hits[0].paper_title.lower())
